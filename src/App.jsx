...
const BUSINESSES = [
  ...
  { id: 57, name: "הפקות יגל&שהם",                cat: "אירועים וצילום",  tel: "052-4225365",   hours: "",               addr: "באר גנים",                  site: "", ig: "https://www.instagram.com/yagel.photographer", fb: "", desc: "צילומי סטילס, מגנטים, אינסטבלוקים לאירועים" },
  ...
  { id: 71, name: "סטלינקה בוטיק בגדים",          cat: "קניות ושירותים",  tel: "050-6525573",   hours: "",               addr: "באר גנים",                  site: "", ig: "", fb: "", desc: "אופנה, סגנון וביטוי – טליה אזולאי" },
...
function MarketCard({ item, isMine, onDelete }) {
  ...
  return (
    <>
      ...
      <div className="card fa vis" style={{ borderTop: `3px solid ${ts.color}`, padding: 0 }}>
        {item.images && item.images.length > 0 && (
          <div onClick={() => setLightbox(true)} style={{ position: "relative", height: 160, background: "#f5ede0", cursor: "zoom-in" }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
            <div style={{ flex: 1 }}>
              <div style={{ display: "flex", gap: 6, marginBottom: 6, flexWrap: "wrap" }}>
                <span style={{ background: ts.bg, color: ts.color, fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 20 }}>{ts.label}</span>
                <span style={{ background: "#f5ede0", color: "#9a7a55", fontSize: 10, padding: "2px 8px", borderRadius: 20 }}>{item.cat}</span>
              </div>
              <h3 style={{ fontSize: 14, fontWeight: 800 }}>{item.title}</h3>
            </div>
            {item.price && item.type === "מכירה" && (
              <div style={{ background: "linear-gradient(135deg,#c4651a,#e8a24e)", color: "#fff", fontWeight: 900, fontSize: 16, padding: "3px 10px", borderRadius: 10 }}>₪{item.price}</div>
            )}
          </div>
          {item.desc && <p style={{ fontSize: 12, color: "#6b5030", marginBottom: 12 }}>{item.desc}</p>}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: 10, color: "#b09070" }}>🕐 {item.date}</span>
            <div style={{ display: "flex", gap: 5 }}>
              {isMine ? (
                <button onClick={() => onDelete(item.id)} style={{ padding: "5px 10px", background: "#fee2e2", color: "#dc2626", border: "none", borderRadius: 20, fontSize: 11, cursor: "pointer" }}>🗑️ מחק</button>
              ) : (
                <a href={`https://wa.me/972${item.tel.replace(/[^0-9]/g, "").replace(/^0/, "")}`} target="_blank" rel="noreferrer" className="ab o" style={{ color: "#16a34a", padding: "5px 10px", fontSize: 11 }}>💬 צור קשר</a>
              )}
            </div>
          </div>
        </div>
...
function AddListingForm({ onAdd, onClose }) {
  ...
  const inp = { width: "100%", padding: "10px", border: "1.5px solid #e8d5b7", borderRadius: 10, marginBottom: 10, fontFamily: "Heebo" };

  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 15 }} onClick={onClose}>
      <div style={{ background: "#fff", borderRadius: 20, padding: 20, maxWidth: 400, width: "100%" }} onClick={e => e.stopPropagation()}>
        <h3 style={{ marginBottom: 15 }}>➕ הוספת מודעה</h3>
        <input style={inp} placeholder="מה מוכרים? *" value={form.title} onChange={e => set("title", e.target.value)} />
        <select style={inp} value={form.cat} onChange={e => set("cat", e.target.value)}>
          {MARKET_CATS.map(c => <option key={c}>{c}</option>)}
        </select>
        <input style={inp} placeholder="מחיר (אם רלוונטי)" type="number" value={form.price} onChange={e => set("price", e.target.value)} />
        <input style={inp} placeholder="טלפון ליצירת קשר *" value={form.tel} onChange={e => set("tel", e.target.value)} type="tel" />
        <textarea style={{ ...inp, height: 60 }} placeholder="תיאור קצר..." value={form.desc} onChange={e => set("desc", e.target.value)} />
        <input type="file" accept="image/*" onChange={handleImages} style={{ marginBottom: 15 }} />
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={() => { if(!form.title || !form.tel) return alert("חובה למלא כותרת וטלפון"); onAdd({...form, images, id: Date.now(), date: new Date().toLocaleDateString("he-IL")}); onClose(); }} style={{ flex: 1, padding: 10, background: "#c4651a", color: "#fff", border: "none", borderRadius: 10, fontWeight: 700 }}>פרסם</button>
          <button onClick={onClose} style={{ flex: 1, padding: 10, background: "#f5ede0", border: "none", borderRadius: 10 }}>ביטול</button>
        </div>
      </div>
    </div>
  );
...
export default function App() {
  ...
  const filtered = useMemo(() => {
    ...
    return BUSINESSES.filter(b => (!q || b.name.toLowerCase().includes(q) || b.desc.toLowerCase().includes(q)) && (activeCat === "הכל" || b.cat === activeCat || (activeCat === "פתוח עכשיו" && getOpenStatus(b.hours) === "open")));
  }, [search, activeCat]);
    ...
    <div style={{ fontFamily: "Heebo, sans-serif", direction: "rtl", minHeight: "100vh", background: "#f7f3ed" }}>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        .si{width:100%;padding:12px 18px;border:2px solid #e8d5b7;border-radius:50px;outline:none;background:#fff;font-family:Heebo}
        ...
        .card{background:#fff;border-radius:18px;padding:15px;border:1.5px solid #ecdfc8;transition:all .2s;cursor:pointer;overflow:hidden}
        .card:hover{transform:translateY(-3px);box-shadow:0 8px 20px rgba(0,0,0,.05)}
        .ab{display:inline-flex;align-items:center;gap:5px;padding:8px 15px;border-radius:50px;font-size:13px;font-weight:600;text-decoration:none;cursor:pointer;border:none}
        .ab.p{background:linear-gradient(135deg,#c4651a,#e8a24e);color:#fff}
        .ab.o{background:#fff;border:1.5px solid #e8d5b7;color:#555}
        .dr{display:flex;align-items:center;gap:8px;padding:6px 0;border-top:1px solid #f5ede0;font-size:13px}
        .fa{opacity:0;transform:translateY(10px);transition:all .3s}
        .fa.vis{opacity:1;transform:translateY(0)}
        .tab-btn{flex:1;padding:15px;border:none;background:none;font-family:Heebo;font-weight:700;font-size:15px;cursor:pointer;color:#f5e6cc;border-bottom:3px solid transparent}
        .tab-btn.act{color:#fff;border-bottom-color:#e8a24e}
        .wa{position:fixed;bottom:20px;left:20px;width:55px;height:55px;border-radius:50%;background:#25d366;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 15px rgba(0,0,0,0.2);text-decoration:none;font-size:25px;z-index:99}
      `}</style>

      <header style={{ background: "linear-gradient(135deg,#1a0d04 0%,#3a2008 100%)", padding: "30px 20px 0", textAlign: "center" }}>
        <div style={{ fontSize: 35 }}>🌿</div>
        <h1 style={{ fontSize: 32, fontWeight: 900, color: "#f5e6cc", marginBottom: 20 }}>עסקים בבאר גנים</h1>
        <div style={{ display: "flex", maxWidth: 960, margin: "0 auto" }}>
          ...
          <button className={`tab-btn ${tab === "market" ? "act" : ""}`} onClick={() => setTab("market")}>🛒 שוק יד 2</button>
        </div>
      </header>

      {tab === "businesses" ? (
        <>
          <div style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(247,243,237,.95)", backdropFilter: "blur(10px)", padding: "15px" }}>
            <input className="si" placeholder="חפש עסק..." value={search} onChange={e => setSearch(e.target.value)} />
          ...
          <main style={{ maxWidth: 960, margin: "0 auto", padding: "15px" }}>
        ...
        <main style={{ maxWidth: 960, margin: "0 auto", padding: "20px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <h2 style={{ fontWeight: 900 }}>🛒 יד שנייה יישובי</h2>
            <button onClick={() => setTab("form")} style={{ padding: "10px 18px", background: "#c4651a", color: "#fff", border: "none", borderRadius: 50, fontWeight: 700, cursor: "pointer" }}>➕ הוסף מודעה</button>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: 15 }}>
            {listings.map(item => <MarketCard key={item.id} item={item} isMine={true} onDelete={id => setListings(l => l.filter(x => x.id !== id))} />)}
            {listings.length === 0 && <p style={{ textAlign: "center", gridColumn: "1/-1", padding: 40, color: "#9a7a55" }}>אין מודעות כרגע. תהיו הראשונים!</p>}
          </div>
        </main>
      )}

      <footer style={{ textAlign: "center", padding: "30px 20px", background: "#ede7da", borderTop: "1px solid #ecdfc8" }}>
        <p style={{ fontWeight: 700, color: "#6b4c2a", fontSize: 16 }}>עסקים בבאר גנים</p>
        <p style={{ marginTop: 10 }}>בעל עסק? רוצה שנבנה לך אתר? <a href="https://wa.me/9720559139013" style={{ color: "#c4651a", fontWeight: 800 }}>לחץ כאן</a></p>
      </footer>

      <a href="https://wa.me/9720559139013" target="_blank" rel="noreferrer" className="wa">💬</a>
      {tab === "form" && <AddListingForm onAdd={item => { setListings(l => [item, ...l]); setTab("market"); }} onClose={() => setTab("market")} />}
    </div>
  );
}
