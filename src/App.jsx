        ...
        <div style={{ padding: 16 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            ...
            <div style={{ display: "flex", gap: 7, flexWrap: "wrap" }}>
              {isMine ? (
                <button onClick={() => onDelete(item.id)} style={{ padding: "7px 12px", background: "#fee2e2", color: "#dc2626", border: "1.5px solid #fca5a5", borderRadius: 50, fontFamily:"'Heebo',sans-serif", fontWeight: 600, fontSize: 12, cursor: "pointer" }}>🗑️ מחק מודעה</button>
              ) : (
                <>
                  <a href={`tel:${item.tel}`} className="ab p" style={{ padding: "7px 14px", fontSize: 12 }}>📞 חיוג</a>
                  <a href={`https://wa.me/972${item.tel.replace(/[^0-9]/g, "").replace(/^0/, "")}`} target="_blank" rel="noreferrer" className="ab o" style={{ color: "#16a34a", borderColor: "#bbf7d0", padding: "7px 14px", fontSize: 12 }}>💬 וואטסאפ</a>
                </>
              )}
            </div>
          </div>
...
function AddListingForm({ onAdd, onClose }) {
  ...
  const submit = () => {
    if (!form.title.trim() || !form.tel.trim()) return alert("נא למלא שדות חובה");
    const now = new Date();
    const date = `${now.getDate()}/${now.getMonth()+1}/${now.getFullYear()}`;
    onAdd({ ...form, images, id: Date.now(), date });
    onClose();
  };
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.55)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
      <div style={{ background: "#fff", borderRadius: 22, padding: 24, maxWidth: 460, width: "100%", direction: "rtl", maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        <h2 style={{ fontSize: 20, fontWeight: 900, color: "#1a0e06", marginBottom: 18 }}>➕ פרסום מודעה</h2>
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          {["מכירה","מתנה בחינם","מחפש/ת"].map(t => (
            <button key={t} onClick={() => set("type", t)} style={{ flex: 1, padding: "9px 4px", borderRadius: 20, border: `2px solid ${form.type===t?"#c4651a":"#e8d5b7"}`, background: form.type===t?"#fff5ee":"#fff", color: form.type===t?"#c4651a":"#7a5c3a", fontFamily:"'Heebo',sans-serif", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>{t}</button>
          ))}
        </div>
        <input style={inp} placeholder="כותרת המודעה *" value={form.title} onChange={e => set("title", e.target.value)} />
        ...
        <input style={inp} placeholder="מספר טלפון ליצירת קשר *" value={form.tel} onChange={e => set("tel", e.target.value)} type="tel" />
        <textarea style={{ ...inp, minHeight: 80 }} placeholder="תיאור נוסף..." value={form.desc} onChange={e => set("desc", e.target.value)} />
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#6b4c2a", marginBottom: 6 }}>הוספת תמונות (עד 4):</label>
          <input type="file" accept="image/*" multiple onChange={handleImages} />
        </div>
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={submit} style={{ flex: 1, padding: "13px", background: "linear-gradient(135deg,#c4651a,#e8a24e)", color: "#fff", border: "none", borderRadius: 14, fontFamily:"'Heebo',sans-serif", fontWeight: 700, cursor: "pointer" }}>פרסם עכשיו</button>
    ...
    <div style={{ fontFamily: "'Heebo',sans-serif", direction: "rtl", minHeight: "100vh", background: "#f7f3ed", color: "#1e140a" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .tab-btn{flex:1;padding:15px;border:none;background:none;font-family:'Heebo',sans-serif;font-weight:700;font-size:16px;cursor:pointer;color:#9a7a55;border-bottom:3px solid transparent}
        .tab-btn.act{color:#c4651a;border-bottom-color:#c4651a}
        .wa{position:fixed;bottom:22px;left:22px;z-index:200;width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,#25d366,#128c7e);display:flex;align-items:center;justify-content:center;box-shadow:0 4px 18px rgba(37,211,102,.5);text-decoration:none;font-size:25px}
        @media(max-width:600px){.grid{grid-template-columns:1fr!important}}
      `}</style>

      <header style={{ background: "linear-gradient(135deg,#1a0d04 0%,#3a2008 55%,#573015 100%)", padding: "32px 20px 42px", textAlign: "center" }}>
        <h1 style={{ fontSize: "clamp(28px,8vw,50px)", fontWeight: 900, color: "#f5e6cc" }}>באר גנים Online</h1>
      </header>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #ecdfc8", display: "flex", maxWidth: 960, margin: "0 auto" }}>
        ...
        <button className={`tab-btn ${tab === "market" ? "act" : ""}`} onClick={() => setTab("market")}>🛒 לוח יד 2 יישובי</button>
      </div>

      {tab === "businesses" ? (
        <>
          <div style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(247,243,237,.97)", backdropFilter: "blur(12px)", padding: "13px 16px" }}>
            <input className="si" type="text" placeholder="חפש עסק..." value={search} onChange={e => setSearch(e.target.value)} />
            <div style={{ display: "flex", gap: 7, overflowX: "auto", marginTop: 10 }}>
              ...
              <button className={`cc open-now ${activeCat === "פתוח עכשיו" ? "act" : ""}`} onClick={() => setActiveCat("פתוח עכשיו")}>🟢 פתוח</button>
        ...
        <main style={{ maxWidth: 960, margin: "0 auto", padding: "20px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <h2 style={{ fontWeight: 900 }}>🛒 השוק של באר גנים</h2>
            <button onClick={() => setShowForm(true)} style={{ padding: "10px 20px", background: "#c4651a", color: "#fff", border: "none", borderRadius: 50, fontWeight: 700, cursor: "pointer" }}>➕ הוסף מודעה</button>
          </div>
          <div style={{ display: "flex", gap: 7, overflowX: "auto", marginBottom: 20 }}>
            {["הכל","מכירה","מתנה בחינם",...MARKET_CATS].map(c => (
              <button key={c} className={`cc ${marketCat===c?"act":""}`} onClick={() => setMarketCat(c)}>{c}</button>
            ))}
          </div>
          <div className="grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(285px,1fr))", gap: 15 }}>
            {filteredListings.map(item => <MarketCard key={item.id} item={item} isMine={true} onDelete={deleteListing} />)}
          </div>
        </main>
      )}

      <footer style={{ textAlign: "center", padding: "40px 20px", background: "#ede7da" }}>
        <p style={{ fontWeight: 700 }}>מעוניינים באתר מקצועי לעסק שלכם?</p>
        <a href="https://wa.me/9720559139013" style={{ color: "#c4651a", fontWeight: 800 }}>לחצו כאן ונדבר!</a>
      </footer>
