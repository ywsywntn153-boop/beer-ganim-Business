    ...
    <>
      ...
      <div className="card fa vis" style={{ borderTop: `3px solid ${ts.color}` }}>
        {item.images && item.images.length > 0 && (
          <div onClick={() => setLightbox(true)} style={{ position: "relative", height: 160, background: "#f5ede0", cursor: "zoom-in", borderRadius: 12, marginBottom: 12, overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 7 }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: "inline-flex", gap: 6, marginBottom: 6, flexWrap: "wrap" }}>
              ...
              <span style={{ background: "#f5ede0", color: "#9a7a55", fontSize: 10, padding: "2px 9px", borderRadius: 20 }}>{item.cat}</span>
            </div>
            <h3 style={{ fontSize: 15, fontWeight: 800, color: "#1a0e06", lineHeight: 1.3 }}>{item.title}</h3>
          </div>
        ...
        {item.desc && <p style={{ fontSize: 13, color: "#6b5030", marginBottom: 9, lineHeight: 1.55 }}>{item.desc}</p>}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid #f0e8d8", paddingTop: 8, marginTop: 8 }}>
          <span style={{ fontSize: 11, color: "#b09070" }}>🕐 {item.date}</span>
          <div style={{ display: "flex", gap: 6 }}>
            <button onClick={() => onDelete(item.id)} style={{ background: "transparent", color: "#dc2626", border: "none", fontSize: 12, cursor: "pointer", padding: "5px" }}>מחק</button>
            <a href={`https://wa.me/972${item.tel.replace(/[^0-9]/g, "").replace(/^0/, "")}`} target="_blank" rel="noreferrer" className="ab o" style={{ color: "#16a34a", padding: "5px 12px", fontSize: 12, borderColor: "#bbf7d0" }}>💬 שלח הודעה</a>
          </div>
        </div>
      </div>
    </>
...
function AddListingForm({ onAdd, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.55)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }} onClick={onClose}>
      <div style={{ background: "#fff", borderRadius: 22, padding: 24, maxWidth: 420, width: "100%", direction: "rtl", maxHeight: "90vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
        <h2 style={{ fontSize: 20, fontWeight: 900, color: "#1a0e06", marginBottom: 18 }}>➕ מודעה חדשה לשוק</h2>
        
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          {["מכירה","מתנה בחינם","מחפש/ת"].map(t => (
            <button key={t} onClick={() => set("type", t)} style={{ flex: 1, padding: "8px 4px", borderRadius: 20, border: `2px solid ${form.type===t?"#c4651a":"#e8d5b7"}`, background: form.type===t?"#fff5ee":"#fff", color: form.type===t?"#c4651a":"#7a5c3a", fontFamily:"'Heebo',sans-serif", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>{t}</button>
          ))}
        </div>

        <input style={inp} placeholder="כותרת למודעה *" value={form.title} onChange={e => set("title", e.target.value)} />
        ...
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#6b4c2a", marginBottom: 6 }}>תמונות להמחשה (עד 2):</label>
      ...
      {tab === 'market' && (
         <main style={{ maxWidth: 960, margin: "0 auto", padding: "20px 14px 80px" }}>
             <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 15 }}>
                <h2 style={{ fontSize: 22, fontWeight: 900, color: "#1a0e06" }}>מודעות אחרונות</h2>
             ...
             {filteredListings.length === 0 ? (
               <div style={{ textAlign: "center", padding: "64px 20px", color: "#b09070" }}>
                 ...
                 <p style={{ fontSize: 18, fontWeight: 700, marginTop: 12 }}>אין עדיין מודעות בקטגוריה זו</p>
               </div>
             ) : (
               <div className="grid" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(285px,1fr))", gap: 13 }}>
