...
function phone(raw) {
  if (!raw) return "";
  const m = String(raw).match(/0\d[\d\-]{7,9}/);
  return m ? m[0] : "";
}
...
const BUSINESSES = [
  ...
  { id: 28, name: "סטודיו me – פילאטיס וכושר",  cat: "בריאות ורפואה",  tel: "050-4442688",   hours: "",              addr: "באר גנים",                 site: "", ig: "", fb: "", desc: "פילאטיס מכשירים, ליווי תזונה, אימוני כושר – מיטל זהבי" },
...
function Stars({ n }) {
  return <span style={{ fontSize: 11, letterSpacing: 0.5 }}>{[1,2,3,4,5].map(i=><span key={i} style={{color: i<=n?"#f59e0b":"#ddd"}}>★</span>)}</span>;
}
...
export default function App() {
  ...
  const filtered = useMemo(()=>{
    ...
    return BUSINESSES.filter(b=>{
      const m = !q || b.name.includes(q)||b.desc.includes(q)||b.addr.includes(q)||b.cat.includes(q);
  ...
  return (
    <div style={{fontFamily:"'Heebo',sans-serif",direction:"rtl",minHeight:"100vh",background:"#f7f3ed",color:"#1e140a"}}>
      ...
      <header style={{background:"linear-gradient(135deg,#1a0d04 0%,#3a2008 55%,#573015 100%)",padding:"32px 20px 42px",textAlign:"center",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{position:"absolute",top:-50,right:-50,width:200,height:200,borderRadius:"50%",border:"1px solid rgba(232,162,78,.1)"}}/>
        <div style={{position:"absolute",bottom:-30,left:-20,width:140,height:140,borderRadius:"50%",border:"1px solid rgba(232,162,78,.08)"}}/>
        <div style={{position:"relative",zIndex:1}}>
          ...
          <div style={{display:"flex",justifyContent:"center",gap:16,marginTop:16,flexWrap:"wrap"}}>
            {Object.entries(CATEGORIES).slice(0,6).map(([c,{emoji}])=>counts[c]?(
              <div key={c} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2}}>
                <span style={{fontSize:20}}>{emoji}</span>
                <span style={{color:"#c9a97a",fontSize:10,fontWeight:500}}>{c}</span>
              </div>
            ):null)}
          </div>
        </div>
      ...
      <div style={{position:"sticky",top:0,zIndex:100,background:"rgba(247,243,237,.97)",backdropFilter:"blur(12px)",borderBottom:"1px solid #ecdfc8",padding:"13px 16px 9px"}}>
        <div style={{position:"relative",maxWidth:520,margin:"0 auto 10px"}}>
          ...
          {search&&<button onClick={()=>setSearch("")} style={{position:"absolute",left:14,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",fontSize:17,cursor:"pointer",color:"#b09070"}}>✕</button>}
        </div>
      ...
      {/* COUNT */}
      <div style={{padding:"9px 18px 2px",maxWidth:960,margin:"0 auto"}}>
        <p style={{color:"#9a7a55",fontSize:13}}>נמצאו <strong>{filtered.length}</strong> עסקים{activeCat!=="הכל"?` בקטגוריה "${activeCat}"`:""}
        </p>
      </div>

      {/* GRID */}
      <main style={{maxWidth:960,margin:"0 auto",padding:"8px 14px 80px"}}>
        {filtered.length===0
      ...
      <footer style={{textAlign:"center",padding:"20px",color:"#b09070",fontSize:13,borderTop:"1px solid #ecdfc8",background:"#ede7da"}}>
        ...
        <p style={{marginTop:6,fontSize:13}}>
          לעדכונים והוספת עסק – צרו קשר:{" "}
          <a href="tel:0559139013" style={{color:"#c4651a",fontWeight:700,textDecoration:"none"}}>055-913-9013</a>
        </p>
        <div style={{marginTop:10,fontSize:13,background:"#fff8f0",display:"inline-block",padding:"9px 18px",borderRadius:12,border:"1px solid #f0d9bb",color:"#6b4c2a"}}>
          🌐 רוצים <strong>אתר לעסק</strong>? פנו ל{" "}
          <a href="tel:0559139013" style={{color:"#c4651a",fontWeight:700,textDecoration:"none"}}>055-913-9013</a>
        </div>
        ...
        <div style={{marginTop:14,fontSize:11,color:"#aaa",maxWidth:480,margin:"14px auto 0",lineHeight:1.6,padding:"10px 14px",background:"#f5ede0",borderRadius:10,border:"1px solid #e8d5b7"}}>
      ...
      {/* WhatsApp FLOAT */}
      <a href={`https://wa.me/972${waFloat.replace(/[^0-9]/g,"").replace(/^0/,"")}`} target="_blank" rel="noreferrer" className="wa" title="צרו קשר בוואטסאפ">💬</a>
    </div>
  );
}
...
function Card({ biz, idx, expanded, onToggle, mounted }) {
  ...
  const hasLink = biz.tel||biz.site||biz.ig||biz.fb;
